...
REPRESENTATION CONVENTION: 
  - Compress represents the compressFile function
  - Decompress represents the deCompressFile function
...
{- compressFile
   PURPOSE: To compress files.
   PRE: The text file the user wants to compress needs to be in the same directory as the program.
        It can only contain ASCII characters and it can't contain ASCII character 127.
...
compressFile = do
  ...
  where
    ...
    {- 
      ...
      PRE: True
      POST: A new file with contents c and the given name f if no naming conflicts occur
            within the directory, otherwise the new file is saved with contents c and a new name.
...
PRE: The text file the user wants to decompress needs to be in the same directory as the program.
     It must be compressed by the function compressFile.
...
makeName name lastName
  ...
  where
    ...
    {- compressed
       ...
       EXAMPLES: name == "hello.txt" == False
                 name == "hellocomp.txt" == True
    -}
    compressed :: Bool 
...
{- intToStr a
   PURPOSE: To convert a list of Ints to its corresponding characters in the ASCII table
   PRE: True
   POST: A string of the corresponding characters given from the list of Ints.
...
{- strToInt str
   PURPOSE: To convert the characters of a string to its corresponding Ints in the ASCII table.
   PRE: True
   POST: A list of the corresponding Ints given from the string.
...
{- makepswd pass
   ...
   PRE: True
   POST: A password consisting of Ints based on the given string.
...
{- asci str
   PURPOSE: To match ASCII characters to its corresponding Ints
   PRE: True
   POST: The Ints matched to its ASCII character. 
